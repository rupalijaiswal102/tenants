...
import { ApproveSignatureModal } from '../tenants/ApproveSignatureModal';
import { InvoicePreviewDocument } from '../invoices/InvoicePreviewDocument';
...
export function ViewInvoiceModal({ invoice, tenant, company, onClose }: Props) {
  const [downloading,    setDownloading]    = useState(false);
  const [showApprove,    setShowApprove]    = useState(false);
  const [currentInvoice, setCurrentInvoice] = useState<any>(invoice);
  ...
  const invoiceItems = (invoice as any).items?.length
