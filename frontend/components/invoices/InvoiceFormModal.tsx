...
// ── Invoice Form Modal ───────────────────────────────────────────────────────
export function InvoiceFormModal({ tenants, companies, otherParties = [], onClose, onSuccess, initialData }: any) {
  const [loading,   setLoading]   = useState(false);
  const [partyType, setPartyType] = useState<'Tenant'|'OtherParty'>(
    initialData?.partyType || (initialData?.otherPartyId ? 'OtherParty' : 'Tenant')
  );
  ...
  const { register, control, handleSubmit, watch, setValue, formState: { errors } } = useForm({
    defaultValues: {
      ...
      otherPartyId: initialData?.otherPartyId || '',
      partyType: initialData?.partyType || 'Tenant',
      ...
      crmName:   (initialData as any)?.crmName  || '',
      crmPhone:  (initialData as any)?.crmPhone || '',
      crmEmail:  (initialData as any)?.crmEmail || '',
  ...
  const handleOtherPartyChange = (id: string) => {
    const party = otherParties.find((p: any) => p.id === id || p._id === id);
    if (party) {
      ...
      if (partyCompanyName) {
        const matchedCompany = companies.find((c: Company) =>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div style={{ display:'flex', gap:8 }}>
                {(['Tenant','OtherParty'] as const).map(type => (
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {partyType==='Tenant' ? (
                  ...
                  >
                    ...
                    {Array.isArray(otherParties) && otherParties.map((p: any) => (
