import { Plus, Eye, History, Trash2 } from 'lucide-react';
...
export default function BillingTab({ invoices = [], onPay, onView, onEdit, onDelete }) {
  return (
          ...
          <thead>
            <tr style={{ background: '#f8f9fb' }}>
              {['Inv No.', 'Bill Date', 'Total', 'Received', 'TDS', 'Balance', 'Status', 'Actions'].map((h, i) => (
                <th key={h} style={{
                  padding: '10px 16px', fontSize: 9, fontWeight: 800, color: '#9ba8b5',
                  textTransform: 'uppercase', letterSpacing: '0.08em',
                  borderBottom: '2px solid #f0f2f5', whiteSpace: 'nowrap',
            ...
            {invoices.length === 0 ? (
              ...
              invoices.map(inv => (
                <tr key={inv.id} style={{ borderBottom: '1px solid #f8f9fb', transition: 'background 0.1s' }}
                  ...
                  <td style={{ padding: '11px 16px', fontSize: 12, fontWeight: 700, color: '#1a1a2e', textAlign: 'right' }}>{fmt(inv.totalInvoice)}</td>
                  <td style={{ padding: '11px 16px', fontSize: 12, fontWeight: 700, color: '#10b981', textAlign: 'right' }}>{fmt(inv.receivedAmount || inv.received)}</td>
                  <td style={{ padding: '11px 16px', fontSize: 12, fontWeight: 700, color: '#8b5cf6', textAlign: 'right' }}>{fmt(inv.tdsAmount)}</td>
                  <td style={{ padding: '11px 16px', fontSize: 12, fontWeight: 700, color: '#ef4444', textAlign: 'right' }}>{fmt(inv.balanceAmount || inv.balance)}</td>
