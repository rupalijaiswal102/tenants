...
export default function TenantList({ mode = 'tenant' }: { mode?: 'tenant' | 'otherParty' }) {
  const { id }   = useParams();
  ...
  const filtered = tenants.filter(t => {
    if (companyFilter !== 'All Companies' && (t.company || (t as any).companyName) !== companyFilter) return false;
    const q  = search.toLowerCase();
    const name = (t.name || '').toLowerCase();
    const company = (t.company || (t as any).companyName || '').toLowerCase();
  ...
  const openView    = (t: Tenant) => { setSelectedTenant(t); setShowDetails(true); navigate(`${basePath}/${t.id}`); };
    ...
    <div style={{ padding: isMobile ? 12 : 24, width: '100%' }}>
      ...
      <div style={{ background:'#fff', borderRadius:16, border:'1px solid #f0f2f5', padding: isMobile ? '10px 12px' : '12px 16px', display:'flex', gap:8, flexWrap:'wrap', alignItems:'center', marginBottom:16, boxShadow:'0 1px 3px rgba(0,0,0,0.04)' }}>
        ...
        <select value={companyFilter} onChange={e=>setCompanyFilter(e.target.value)}
          ...
          {[...new Set(tenants.map(t=>t.company||(t as any).companyName).filter(Boolean))].sort().map((c:string)=>(
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={t.id} onClick={() => openView(t)}
                  ...
                  <td style={{ padding:'14px 18px' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:11 }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize:10, color:'#9ba8b5', margin:'2px 0 0' }}>{t.email || (t as any).companyName || ''}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  {/* Property / Address */}
                  <td style={{ padding:'14px 18px' }}>
                    <p style={{ fontSize:12, color:'#5a6474', margin:0 }}>{t.property || (t as any).address || '—'}</p>
                  </td>
                  ...
                  <td style={{ padding:'14px 18px' }} onClick={e => e.stopPropagation()}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:6 }}>
                      ...
                      <button onClick={() => navigate(`${basePath}/edit/${t.id}`)}
