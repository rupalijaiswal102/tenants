...
// ── Summary compute ───────────────────────────────────────────────────────────
function computeSummary(invoices: Invoice[], type: string) {
  const totalInvoiced  = invoices.reduce((s, i) => s + (i.totalInvoice || 0), 0);
  const totalReceived  = invoices.reduce((s, i) => s + (i.receivedAmount || (i as any).received || 0), 0);
  const totalBalance   = invoices.reduce((s, i) => s + (i.balanceAmount || (i as any).balance || 0), 0);
...
// ── PDF Generator ─────────────────────────────────────────────────────────────
function generatePDF(invoices: Invoice[], reportType: string, month: string, year: string) {
  ...
  doc.text(`Total Invoiced: Rs ${sum.totalInvoiced.toLocaleString('en-IN')}`, 220, 18, { align: 'right' });
  doc.text(`Total Received: Rs ${sum.totalReceived.toLocaleString('en-IN')}`, 220, 24, { align: 'right' });
  ...
  const rows = invoices.map((inv, i) => ({
    ...
    received: `Rs ${(inv.receivedAmount || (inv as any).received || 0).toLocaleString('en-IN')}`,
    balance:  `Rs ${(inv.balanceAmount || (inv as any).balance || 0).toLocaleString('en-IN')}`,
    ...
    net:      `Rs ${(inv.receivedAmount || (inv as any).received || 0).toLocaleString('en-IN')}`,
    status:   inv.paymentStatus,
  }));

  // Manual table drawing
  let ty = 58;
  const colW = cols.map((c: any) => c.dataKey === 'party' ? 50 : c.dataKey === 'no' ? 10 : 30);
  const totalW = colW.reduce((a: number, b: number) => a + b, 0);
  const startX = 14;
  ...
  const handleExcel = () => {
    ...
    try {
      const rows = filtered.map(inv => ({
        ...
        'Received':   inv.receivedAmount || (inv as any).received || 0,
        'Balance':    inv.balanceAmount  || (inv as any).balance  || 0,
                  ...
                  return (
                    <motion.tr key={inv.id} initial={{ opacity:0, y:4 }} animate={{ opacity:1, y:0 }} transition={{ delay:idx*0.02 }}
                      ...
                      {reportType==='collection' && <>
                        <td style={{ padding:'11px 16px', fontSize:12, fontWeight:700, color:'#10b981', textAlign:'right', whiteSpace:'nowrap' }}>₹{(inv.receivedAmount||(inv as any).received||0).toLocaleString('en-IN')}</td>
                        <td style={{ padding:'11px 16px', fontSize:12, fontWeight:700, color:'#ef4444', textAlign:'right', whiteSpace:'nowrap' }}>₹{(inv.balanceAmount||(inv as any).balance||0).toLocaleString('en-IN')}</td>
                      </>}
                      {reportType==='pending' && <td style={{ padding:'11px 16px', fontSize:12, fontWeight:700, color:'#ef4444', textAlign:'right', whiteSpace:'nowrap' }}>₹{(inv.balanceAmount||(inv as any).balance||0).toLocaleString('en-IN')}</td>}
                      {reportType==='gst' && <>
